...
function blankSentence(sentence: string, hiddenWord: string) {
  const placeholder = "___";
  if (sentence.includes(hiddenWord)) {
    return sentence.replace(hiddenWord, placeholder);
  }
  const words = sentence.split(/\s+/);
  return words.map((w) => (w === hiddenWord ? placeholder : w)).join(" ");
}
...
export default function GrammarFillBlank({ items, onComplete }: Props) {
  ...
  const options = item ? shuffle([item.hiddenWord, ...item.wrongWords]) : [];

  const handleAnswer = (answer: string) => {
    if (selected || showResult) return;
    setSelected(answer);
    const correct = answer === item.hiddenWord;
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <Card className="rounded-2xl border-border">
        <CardContent className="p-6 space-y-6">
          <p className="text-2xl font-black text-foreground leading-loose text-center">
            {blankSentence(item.sentence, item.hiddenWord)}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {options.map((option) => {
              const isSelected = selected === option;
              const isCorrect = option === item.hiddenWord;
