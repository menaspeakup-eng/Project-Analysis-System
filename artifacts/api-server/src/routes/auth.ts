...
async function upsertUser(claims: Record<string, unknown>) {
  ...
  // Keep the platform's student row in sync with the Replit Auth profile.
  // Match by replitUserId first, then by email, so existing Clerk-only rows
  // can be migrated automatically when the user signs in with the same email.
  const email = normalizeEmail(userData.email);
  let student = await db.query.studentsTable.findFirst({
    where: or(
      eq(studentsTable.replitUserId, user.id),
      email ? eq(studentsTable.email, email) : undefined,
    ),
  });
  ...
  if (!student) {
    ...
    if (!student.replitUserId) updates.replitUserId = user.id;
    if (!student.email && email) updates.email = email;
    if (student.email !== email && email) updates.email = email;
    if (student.imageUrl !== userData.profileImageUrl) {
      updates.imageUrl = userData.profileImageUrl;
    }
    // If the user never confirmed their name, keep syncing with the auth provider.
    if (!student.nameConfirmed && student.name !== displayName) {
