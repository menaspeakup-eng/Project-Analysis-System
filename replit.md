...
- Avatar customization is a real 3D system (`/character`): background color + gender + one pet + **one** accessory, rendered with react-three-fiber. Each gender+accessory combo is its own fully pre-modeled GLB (character and accessory sculpted together by the 3D generator), not a generic body with a separate accessory piece composited on top at runtime — that runtime-compositing approach couldn't reliably size/place generated accessory props, so only one accessory can be worn at a time now. Locked accessories/pets can be tapped to preview on the 3D model without unlocking or saving them.
